...
4. **`SUPPORTED_VERSIONS` のリプレイ攻撃**: timestamp tolerance (past 300s / future 60s) 内で同一 body + 同一 timestamp + 同一 signature の重複送信に対し、Worker / DB 両者に冪等性検証は無い (`worker/src/index.ts` で nonce / dedup なし、`yakiimo_temp_logs` に unique 制約なし `supabase/migrations/001_create_yakiimo_temp_logs.sql:5-16`)。同一行が複数回 INSERT される。
