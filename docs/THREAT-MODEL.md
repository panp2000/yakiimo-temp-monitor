...
3. **TLS 証明書検証なし (ESP32 → Worker)**: `firmware-esphome/yakiimo.yaml:113` で `verify_ssl: false`。**Phase 14-B で defer 判断 (`docs/adr/0003-defer-tls-ca-pinning.md`)**。ESPHome の `http_request_arduino.cpp` が HTTPS 接続時に無条件で `WiFiClientSecure::setInsecure()` を呼び出す実装のため、Arduino framework では yaml レベル / C++ レベルどちらでも CA 固定は実現不可。ESP-IDF framework 切替が必要となるが、現状の脅威モデル (公開リポ + 焼き芋屋 portfolio、HMAC envelope による integrity 担保) でこのリスクは許容。完全な TLS MITM 攻撃下では body 平文を観測されるが、HMAC があるため payload 改竄や偽装 ingest は防げる。ADR-0003 の trigger 条件 (Arduino で CA pinning がサポートされた時 / 機密 body 追加時 / ESP-IDF 切替リファクタ時 / MITM 攻撃の実例観測時) で再評価する。
